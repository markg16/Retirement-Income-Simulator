classdef Scenario <scenarios.ScenarioDataProvider
    ...
    methods
      ...
      function dateLastPrices = getDateLastPrices(obj)
        ...
        function obj = generateScenarioMarketData(obj,marketData,scenarioMarketSimulationParameters)
            ...
            % OUTPUT 
            % output is a market data object with data relevant to the
            % scenario
            ...
            % extract required historical market data from the market data
            % object
            ...
            tempScenario = obj.generateScenarioMarketSimulationParameters(scenarioStartDate,simulationStartDate,scenarioEndDate,tempScenarioMarketData);
        ...
        function obj = generateScenarioMarketSimulationParameters(obj,scenarioStartDate,simulationStartDate,scenarioEndDate,scenarioMarketData)
            
            marketDataAssetNames = scenarioMarketData.ScenarioHistoricalMarketData.MarketIndexPrices.Properties.VariableNames;
            ...
            simulationStartValues = scenarioMarketData.getMarketPrices(simulationStartDate); % make we return tickers as headers
            ...
            scenarioMarketSimulationParameters.SimulationIdentifier = scenarioMarketData.SimulationIdentifier;
          ...
          % make sure that Risk Premiums are returned with Tickers as
          % headers and we return same number of risk premiums as tickers
          % in the data. This means we need to ste a defualt ticker
          % assumption using some sort of categorisation of the indices.
            scenarioMarketSimulationParameters.ForwardRates = scenarioMarketData.getScenarioForwardRates(simulationStartDate,assetReturnStartDates,assetReturnEndDates,scenarioMarketSimulationParameters.Country,scenarioMarketSimulationParameters.RateScenarios);
