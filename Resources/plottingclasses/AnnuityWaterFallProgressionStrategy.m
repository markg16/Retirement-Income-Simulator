...
classdef AnnuityWaterFallProgressionStrategy < PlotStrategy
    ...
    methods
        % function plot(obj, axesMap, plotData, ~) % Use ~ to ignore the unused plotConfig
        ...
        %   progressionTimetable: A timetable generated by Annuity.calculateProgression().



        % cla(ax, 'reset'); % Clear the axes first

        % function plot(obj, axesMap, plotData, plotConfig) % Use ~ to ignore the unused plotConfig
        %     % For this plot, we assume the data is for a single annuity type
        %     % and contains the progression data. We will plot the first
        %     % progression found onto the first available axes.
        % 
        %     if isempty(plotData) || isempty(plotData(1).Data)
        %         return;
        %     end
        % 
        %     annuityTypeData = plotData.Data;   %  allAnnuityValues(1);
        %     annuityTypeName = plotData.AnnuityType;   %   annuityTypeData.AnnuityType;
        % 
        %     if ~axesMap.isKey(annuityTypeName)
        %         warning('WaterfallPlotStrategy:MissingAxes', 'No axes handle for AnnuityType "%s".', annuityTypeName);
        %         return;
        %     end
        %     ax = axesMap(annuityTypeName);
        % 
        %     % Get the first progression timetable from the analysis results
        %     progressionTimetable = annuityTypeData.ProgressionData{1};
        % 
        %     % Use the plotting utility to create the waterfall chart
        %   utilities.PlottingUtils.plotAnnuityProgressionWaterfall(ax, progressionTimetable);
        % 
        % 
        % end
        function plot(obj, axesMap, allAnnuityValues, plotConfig)
            ...
            for j = 1:length(annuityTypesToPlot)
                ...
                for i = 1:numPeriods
                    ...
                    arrowYPos = max(startBarTop, endBarTop) * 1.08; % Position arrow above the bars

                    % Draw the horizontal line of the arrow
                    plot(ax, [x_offset, endBarPosition], [arrowYPos, arrowYPos], 'k-', 'LineWidth', 1.5);
                    % Draw the left and right arrowheads as markers
                    plot(ax, x_offset, arrowYPos, '<k', 'MarkerFaceColor', 'k', 'MarkerSize', 8);
                    plot(ax, endBarPosition, arrowYPos, '>k', 'MarkerFaceColor', 'k', 'MarkerSize', 8);
                    ...
                    text(ax, x_offset + (endBarPosition - x_offset)/2, arrowYPos, arrowText, ...
                        'HorizontalAlignment', 'center', ...
